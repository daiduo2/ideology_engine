...
# -*- coding: utf-8 -*-
"""
MBTI Personality Assessment Demo - FAST VERSION with parallel processing.

This version uses multi-agent architecture for near-instant question generation.
...
async def get_input_claude_style():
    ...
    - 多行输入支持（Alt+Enter 或 Esc+Enter 换行）
    - 上下箭头浏览历史
    - 左右箭头移动光标
    - Backspace/Delete 正常删除
    - 良好的中文输入支持
    """
    try:
...
async def main():
    print("=" * 70)
    print("🧠 MBTI 人格评估 - 极速版 (并行处理)")
    print("=" * 70)
    print("\n⚡ 特性: 证据提取和问题生成并行执行，响应速度提升 3-5 倍")
    ...
    # Initialize parallel engine with 3 workers
    ...
    print("\n" + "-" * 70)
    print("💬 评估开始！回答后立即显示下一题，后台异步处理证据。")
    print("   输入 'quit' 可随时退出。")
    print("-" * 70)
    ...
    try:
        for round_num in range(max_rounds):
            # Get next question (fast - evidence processed in background)
            q_start = time.time()
            ...
            # Get user input with Claude Code style
